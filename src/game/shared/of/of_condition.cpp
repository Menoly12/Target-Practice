...
COFCondition::COFCondition( 
	EOFCond nCond, float flDuration, CCondPlayer *hOuter, int iSeed,
...
void COFCondition::Think( void )
{
	...
	if ( GetRemainingDuration() != PERMANENT_LENGTH )
	{
...
// flDuration - How long the cond will be applied for, PERMANENT_LENGTH(-1) for infinite
...
//-----------------------------------------------------------------------------
void COFCondManager::AddCond( EOFCond nCond, float flDuration /* = PERMANENT_LENGTH */, EOFCondStackingMode nCanStack /* = OF_COND_STACK_NONE */,
	CBaseEntity *pProvider /* = NULL */, char *szVars /* = NULL */ )
{
	Assert( nCond >= 0 && nCond < OF_COND_LAST );
...
//-----------------------------------------------------------------------------
void COFCondManager::RemoveCond( EOFCond nCond, CBaseEntity *pProvider )
{
	Assert( nCond >= 0 && nCond < OF_COND_LAST );
...
COFCondition *COFCondManager::GetCond( EOFCond nCond, CBaseEntity *pProvider /* = NULL*/, COFCondition *pPrevious /*= NULL*/ )
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool COFCondManager::InCond( EOFCond nCond )
{
	Assert( nCond >= 0 && nCond < OF_COND_LAST );
...
// Purpose:
//-----------------------------------------------------------------------------
float COFCondManager::GetConditionDuration( EOFCond nCond )
{
	Assert( nCond >= 0 && nCond < OF_COND_LAST );
...
void (*g_fnCondThink[OF_COND_LAST])( COFCondition *pCond ) =
{
	NULL, // TF_COND_AIMING
	NULL, // TF_COND_ZOOMED
