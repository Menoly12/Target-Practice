...
BEGIN_NETWORK_TABLE_NOBASE(COFPlayerShared, DT_OFPlayerSharedLocal)
#ifdef CLIENT_DLL
...
//RecvPropFloat(RECVINFO(m_flStealthNoAttackExpire)),
//RecvPropFloat(RECVINFO(m_flStealthNextChangeTime)),
//RecvPropArray3(RECVINFO(m_bPlayerDominated), RecvPropBool(RECVINFO(m_bPlayerDominated[0]))),
//RecvPropArray3(RECVINFO(m_bPlayerDominatingMe), RecvPropBool(RECVINFO(m_bPlayerDominatingMe[0]))),
#else
#endif
END_NETWORK_TABLE()
...
//RecvPropInt(RECVINFO(m_iAirDucked)),
...
//m_nArenaNumChanges
// econ...
//m_flInvisChangeCompleteTimes
//m_nDisguiseTeam
//m_nDisguiseClass
//m_nDisguiseSkinOverride - cant you just use the diguise team
//m_nMaskClass - cant you just use the disguise class >:?
...
//m_flCloakMeter
//m_flSpyTranqBuffDuration - interesting

RecvPropDataTable("ofsharedlocaldata", 0, 0, &REFERENCE_RECV_TABLE(DT_OFPlayerSharedLocal)),
#else
...
SendPropInt(SENDINFO(m_nPlayerState), 3, SPROP_UNSIGNED),
#endif
END_NETWORK_TABLE()
...
{
	float flCritCalc = m_iCritMult * 0.003921569;
...
void COFPlayerShared::OnConditionAdded( EOFCond nCond )
{
/*
  char cVar1;
...
void COFPlayerShared::OnConditionRemoved( EOFCond nCond )
{
	/*
  CTFPlayer *pCVar1;
  char cVar2;
...
void COFPlayerShared::AddCond(EOFCond nCond, float flDuration, EOFCondStackingMode nCanStack, CBaseEntity *pProvider, char *szVars)
{
	m_Conds.AddCond(nCond, flDuration, nCanStack, pProvider, szVars);
}

void COFPlayerShared::RemoveCond(EOFCond nCond, CBaseEntity *pProvider)
{
	m_Conds.RemoveCond(nCond, pProvider);
}

bool COFPlayerShared::InCond(EOFCond nCond)
{
	return m_Conds.InCond(nCond);
}
...
bool COFPlayerShared::IsLoser()
{
	if (tf_always_loser.GetBool()) return true;

	if (!OFGameRules()) return false;

	bool bLoser = false;

	if (OFGameRules()->State_Get() == GR_STATE_TEAM_WIN)
	{
		bLoser = OFGameRules()->GetWinningTeam() != m_pOuter->GetTeamNumber();

		// if the losing spy is disguised as the winning team, dont make them count as a loser
		if (bLoser && m_pOuter->m_Class.GetClassIndex() == OF_CLASS_SPY)
		{
			//if (InCond(OF_COND_DISGUISED) && m_nDisguiseTeam == OFGameRules()->GetWinningTeam())
			//{
			//	bLoser = false;
			//}
		}
...
void COFPlayer::SetSpeedOF()
{
	...
	if (m_Shared.InCond(OF_COND_AIMING))
	{
...
{
	if (m_Shared.InCond(OF_COND_DISGUISING) || m_Shared.InCond(OF_COND_DISGUISED))
	{
