...
#define COND_THINK_COUNT OF_COND_LAST
...
public:
	...
	COFCondition();
	COFCondition( EOFCond nCond, float flDuration, CCondPlayer *hOuter, int iSeed, EOFCondStackingMode nCanStack = OF_COND_STACK_NONE, CBaseEntity *hProvider = NULL, char *szVars = NULL );
	~COFCondition();

	EOFCond GetID() { return (EOFCond)m_iID.Get(); };
...
public:
	...
	void	AddCond( EOFCond nCond, float flDuration = PERMANENT_LENGTH, EOFCondStackingMode nCanStack = OF_COND_STACK_NONE, CBaseEntity *hProvider = NULL, char *szVars = NULL );
	void	RemoveCond( EOFCond nCond, CBaseEntity *pProvider = NULL );
	void	RemoveCondSeed( int iSeed );

	COFCondition *GetCond( EOFCond nCond, CBaseEntity *pProvider = NULL, COFCondition *pPrevious = NULL );
	bool		InCond( EOFCond nCond );
	void		RemoveAllCond( CBaseEntity *pProvider = NULL );
	float		GetConditionDuration( EOFCond nCond );
